...
The B is a single closed path: up the stem, round the upper bowl back to the
middle, round the lower bowl back to where it started. It's a lap that happens
to spell the letter, with the orange dot marking where you set off. The opening
screen draws that path stem-first, so you watch the road being laid.

It's defined once as an SVG path — `M31 83 L31 17 A34 16.5 0 0 1 31 50 A40
16.5 0 0 1 31 83 Z` — and the PNG icons come from the same geometry, so
changing one means changing both.

If the script fails to run, a CSS animation clears the opening screen after six
seconds. Without it a broken build would look like a dead app rather than a
broken one.

## The search doesn't block

The 200 attempts run in 20ms slices between animation frames rather than one
long loop. Two things follow. The map stays draggable while it thinks, instead
of the whole page freezing. And the best lap found so far is drawn as soon as
there is one — typically within a second — then quietly replaced as better ones
turn up, so the wait is something to watch rather than a spinner.

The map deliberately holds still after that first draw; refitting the view on
every improvement would throw it around several times a second.

`findCircuits` (straight through) and `findCircuitsLive` (sliced) share one set
of jobs, so they do identical work and return identical results. The sliced one
just decides when to stop and breathe.

## Opening on a button, not a form

Your last time, reach and corner style are kept in `backroads.prefs` and put
back when you open the app, chips included. So the first screen is a sentence
saying what's about to happen — "About an hour of driving, starting within 10
miles" — and one button. Options is a tap away when you want it.

Find me a drive is the only action until a lap exists. Once one does, the Route
tab belongs to that lap and shows Navigate and the star, with searching moved
to Options where changing your mind belongs.

## Telling where a lap goes

Under the figures, each lap says which settlements it runs through, in the
order you meet them, and which roads carry most of it. Both come from data
already being fetched: every OSM way carries its name, and settlements are one
extra clause on the Overpass query.

How close you have to pass to count depends on the size of the place — 1.5km
for a town, 450m for a hamlet — because you'd say you went "through Southam"
from its edge, but not "through" a hamlet you passed a field away from. Saved
laps are named after the largest place they pass, so you get "Southam loop"
rather than "24 mile lap".

The same places are pinned on the map itself, with a halo behind the text so
they stay readable over roads, open country and the route line in either theme.
Saved laps store their labels, so reopening one months later still shows them
even though the road network it came from is long gone.

It's worked out only for the lap on screen. Doing it for all 200 candidates
would cost more than the search.

## How long a lap takes

You ask for time, not distance, because time is how you actually decide to go
for a drive. "I've got an hour" is a real thought; "I want 15 miles" isn't.

The duration isn't distance divided by an average. Every stretch gets a speed
profile: at each point along it the speed is capped by whichever is lowest of
the limit, the bend (`v = sqrt(A_LAT * radius)`), and what the road class can
realistically carry — a national-limit single-track lane is still a 35mph road.
Those caps then get a forward pass for acceleration and a backward pass for
braking, so a hairpin slows the approach and the exit too, not just the apex.
Turning off one road onto another costs time on top.

The constants live near `edgeSeconds`:

    A_LAT   3.8 m/s²   cornering grip used — about 0.39g
    A_ACC   1.8 m/s²   how hard you accelerate out
    A_DEC   2.6 m/s²   how hard you brake in
    CLASS_SPEED        practical top speed per road type

### Calibrating it

You don't have to edit any of this by hand. After driving a lap, tap the
stopwatch on its row in the saved list (or answer the prompt that appears) and
give it the actual time. It stores the ratio and corrects every estimate from
then on, including the target it searches against — ask for an hour and you get
a lap that takes you an hour, not one that takes the model an hour.

It keeps the last ten timings and uses the **median**, so one drive where you
stopped for fuel doesn't drag everything with it, and the factor is clamped
between 0.6 and 1.8 so a mistyped number can't break it. "Forget my timings"
in Options resets it. The timings live separately from the laps, so deleting a
lap doesn't discard what it taught the app.

One timed drive is enough to converge. More just makes it steadier.

**The constants below are estimates, not measurements.** They're set for brisk driving on
roads you don't know. If the app says 50 minutes and the drive takes an hour,
the fix is to lower `A_LAT` and the `CLASS_SPEED` values by roughly the same
proportion you were out by. If you're consistently quicker than it predicts,
raise them. One drive with a stopwatch is worth more than any amount of
guessing here.

Overshooting the time you asked for is penalised nearly twice as hard as
undershooting, on the grounds that an hour means an hour and a lap that
quietly runs to ninety minutes is a broken promise.
